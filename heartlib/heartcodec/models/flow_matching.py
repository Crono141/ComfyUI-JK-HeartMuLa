...
class FlowMatching(nn.Module):
    ...
    def inference_codes(
        ...
        quantized_feature_emb = (latent_masks > 0.5).unsqueeze(
            -1
        ).to(dtype) * quantized_feature_emb + (latent_masks < 0.5).unsqueeze(
            -1
        ).to(dtype) * zero_cond_embedding1_casted.unsqueeze(
            0
        )

        incontext_latents = (
            true_latents
            * ((latent_masks > 0.5) * (latent_masks < 1.5)).unsqueeze(-1).to(dtype)
        )
        ...
        t_span = torch.linspace(
            0, 1, num_steps + 1, device=quantized_feature_emb.device, dtype=dtype
        )
        latents = self.solve_euler(
            latents * torch.tensor(temperature, device=device, dtype=dtype),
            incontext_latents,
            incontext_length,
            t_span,
