...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x * torch.rsqrt(var + x.new_tensor(self.eps))
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_sdpa and self._has_sdpa:
            ...
            attn_scores = torch.matmul(q, k.transpose(-2, -1)) / q.new_tensor(math.sqrt(self.head_dim))
            if attention_mask is not None:
...
class LlamaTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_ada_layer_norm_single:
            ...
            norm_hidden_states = norm_hidden_states * (scale_msa.new_tensor(1.0) + scale_msa) + shift_msa
            ...
            norm_hidden_states = norm_hidden_states * (scale_mlp.new_tensor(1.0) + scale_mlp) + shift_mlp
...
class ProjectLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x * x.new_tensor(self.kernel_size**-0.5)
        x = self.ffn_2(x)
        return x
...
class LlamaTransformer(nn.Module):
    ...
    def forward(
        ...
        s = self.norm_out(s)
        s = s * (scale.new_tensor(1.0) + scale) + shift
        ...
        x = self.norm_out_2(x)
        x = x * (scale_2.new_tensor(1.0) + scale_2) + shift_2
