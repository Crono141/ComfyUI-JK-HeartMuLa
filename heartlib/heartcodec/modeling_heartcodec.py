...
class HeartCodec(PreTrainedModel):
    ...
    def detokenize(
        ...
        first_latent = torch.randn(codes.shape[0], int(duration * 25), 256, device=device, dtype=dtype)
        ...
        for sinx in tqdm(range(0, codes.shape[-1] - hop_samples + 1, hop_samples), desc="Decoding Audio Chunks", disable=disable_progress):
            codes_input = []
